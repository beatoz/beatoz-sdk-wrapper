...
import { BeatozTxSigner, BeatozChain, BeatozEvmEventService, ContractJson } from '../sdk-wrap';
...
export class TokenBtip10Core extends BaseErc20Client {
  ...
  constructor(
    ...
    btip10TokenContractJson: ContractJson,
    linkerEndpointContractJson: ContractJson
  ) {
  ...
  async setLinkerEndpoint(ownerAccount: BeatozTxSigner, endpointContractAddress: string) {
  ...
  async postAmount(fromAccount: BeatozTxSigner, toChainId: string, toDAppAddr: string, toAccount: string, amount: string) {
